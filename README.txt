...
If a user desires to make corrections for their own custom library for an individual librarythen they may utilize CSC hg38 (and soon mm10) genome-wide pickle files. Genome wide pickles are located in the following repository.
...
If the custom file contains both gRNA sequences and a numerical metric (ie: logFC or other output metric) in the first two columns then CSC adjustments will be modeled and made. If only gRNA sequences are present then the output will be the off-target space up to a Hamming distance of 3 and the GuideScan specificity score for each gRNA
